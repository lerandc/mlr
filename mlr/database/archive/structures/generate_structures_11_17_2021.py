...
def main():
    ...
    for i in tqdm(range(N_particles)):
        ...
        # thickness is uniform between 20 and 80 angstroms
        thickness = rng.uniform(20,80)
        sub_rect_prism = makeRectPrism(128,128,thickness)
        substrate = Volume(points=sub_rect_prism, generator=substrate_gen)
        substrate.priority = 1

        ## set up Au volume
        r = get_radius(rng=rng, mean=40, std=10, lb=30, ub=50)[0]
        sphere = Sphere(center=np.array([0,0,0]), radius=r)
        vol = Volume(alg_objects=[sphere])

        N_defects = rng.integers(low=0, high=3, endpoint=True)
        ...
        scene = Scene(bounds=np.array([[0,0,0],[128,128,thickness+80]]), objects=[current_vol])
        scene.populate()
        ...
        metadata["substrate_thickness"] = thickness
        if N_defects == 0:
