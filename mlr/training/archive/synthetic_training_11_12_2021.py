...
def main():
    ...
    #params include
    batch_size = 16
    target_dose = [[600], [300, 600, 900], [100, 300, 600, 900, 1200]]
    N_defocus = [1, 2, 4]
    backbone = ["resnet18"]
    alpha_0 = [0.01] #starting learning rate
    schedule = [(0.8, "every_epoch")]

    param_list = list(product(*(target_dose, backbone, alpha_0, schedule, N_defocus)))
...
def run_training(**kwargs):
    ...
    df = df[df['dose'].isin(target_dose)]

    N_defocus = kwargs['N_defocus']
    if N_defocus == 1:
        df = df.drop_duplicates(subset=["particle_ID", "dose"])
    elif N_defocus == 2:
        # keep two occurences of each particle
        df = pd.concat([df.drop_duplicates(subset=["particle_ID", "dose"], keep="first"),
                        df.drop_duplicates(subset=["particle_ID", "dose"], keep="last")])
    elif N_defocus == 4:
        pass
