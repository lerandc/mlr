...
def run_training(**kwargs):
    ...
    unique_ids = df["particle_ID"].drop_duplicates()
    # print("Number of images: ", len(df))
    # print("Number of filtered images: ", len(df[df["particle_ID"].isin(unique_ids[0:kwargs["N_structures"]])]))

    df = df[df["particle_ID"].isin(unique_ids[0:kwargs["N_structures"]])]
    df = df.sample(frac=kwargs["N_defocus"]/11.0, random_state=kwargs["seed"])
